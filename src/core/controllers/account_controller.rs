...
use crate::{core::{data_model::{implementations::AccountSession, traits::{IAccountRelated, IAccountSession, ILocalObject}}, services::{get_account_by_id, sign_in_by_refresh_token, sign_in_by_user_creditials}}, AppState};
...
pub async fn sign_in(params: Query<HashMap<String, String>>, State(state) : State<AppState>) -> String {
    ...
    if account_session_option.is_none() { return String::from("NONE"); }

    let account_session = account_session_option.unwrap();

    return String::from(format!("access_token: {}\nrefresh_token: {}\n", account_session.auth_token(), account_session.refresh_token()));
}