...
pub async fn create_account_session(id : &str, account_id : &str, state : &AppState) -> () {
    ...
    let account_session = AccountSession {
     ...
     let account_session_exists = is_account_session_already_exists_by_id(&account_session.id.as_str(), &state).await;
...
pub async fn get_account_session_by_id(id : &str, state : &AppState) -> impl IAccountSession {
