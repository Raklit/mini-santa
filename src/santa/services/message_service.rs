...
fn row_to_message(row : &SqliteRow) -> Message {
    ...
    return Message::new(id, text_content, account_id, room_id, creation_date);
}
...
pub async fn create_message(id : &str, text_content : &str, account_id : &str, room_id : &str, state : &AppState) -> () {
    ...
    let mut context = tera::Context::new();
     ...
     context.insert("account_id", account_id);
     context.insert("room_id", room_id);
     context.insert("creation_date", creation_date.as_str());
